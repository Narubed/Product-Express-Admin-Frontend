...
import PrintInvoiceFullA4 from "@/components/pages/report/been-cut/PrintInvoiceFullA4";
...
export default function BeenCut() {
  ...
  const fetcherPreOrders = async () => {
    const url = `${process.env.NEXT_PUBLIC_PRODUCT_EXPRESS_BACKEND}/pre_orders/cut-around/${id}`;
    const urlMembers = `${process.env.NEXT_PUBLIC_PRODUCT_EXPRESS_BACKEND}/members`;
    let members = [];
    let preorders = [];
    await fetcherWithToken(url).then((json) => {
      preorders = json.data;
    });
