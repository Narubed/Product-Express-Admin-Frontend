...
import HeaderSubmitEdit from "@/components/pages/partners/store/edit/handleSubmitEdit";
import ChangePassword from "@/components/pages/partners/store/edit/changePassword";
  ...
  const fetcherPartners = async () => {
    ...
    await fetcherWithToken(url)
      .then((json) => {
        ...
        setValues({
          ...values,
          CardImage: json.data.partner_card_image,
          BookBankImage: json.data.partner_bookbank_image,
          name: json.data.partner_name,
          email: json.data.partner_email,
          status: json.data.partner_status,
          phone: json.data.partner_phone,

          Thai: json.data.partner_name_center.Thai,
          Eng: json.data.partner_name_center.Eng,
          Cambodia: json.data.partner_name_center.Cambodia,
          Myanmar: json.data.partner_name_center.Myanmar,
          Laos: json.data.partner_name_center.Laos,
          China: json.data.partner_name_center.China,

          AddressThai: json.data.partner_address.Thai,
          AddressEng: json.data.partner_address.Eng,
          AddressCambodia: json.data.partner_address.Cambodia,
          AddressMyanmar: json.data.partner_address.Myanmar,
          AddressLaos: json.data.partner_address.Laos,
          AddressChina: json.data.partner_address.China,
        });
      })
      .catch((err) => {
        router.push("/404");
        ...
        <form onSubmit={handleSubmit}>
          ...
          >
            <Grid item xs={12} sx={{ marginBottom: 3 }}>
              ...
              >
                {fileCard.length === 0 ? (
                  values.CardImage === "ไม่มี" ? (
                    <ImgStyled src={imagesicon.src} alt={imagesicon.src} />
                  ) : (
                    <ImgStyled
                      src={`${process.env.NEXT_PUBLIC_PRODUCT_EXPRESS_BACKEND}/static/images/partners/${values.CardImage}`}
                      alt={`${process.env.NEXT_PUBLIC_PRODUCT_EXPRESS_BACKEND}/static/images/partners/${values.CardImage}`}
                    />
                  )
                ) : (
                ...
                {fileBookBank.length === 0 ? (
                  values.CardImage === "ไม่มี" ? (
                    ...
                    <ImgStyled
                      src={`${process.env.NEXT_PUBLIC_PRODUCT_EXPRESS_BACKEND}/static/images/partners/${values.BookBankImage}`}
                      alt={`${process.env.NEXT_PUBLIC_PRODUCT_EXPRESS_BACKEND}/static/images/partners/${values.BookBankImage}`}
                    />
                  )
                ) : (
