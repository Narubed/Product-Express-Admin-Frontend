...
export default function CountrySelect({
  ...
  const [isSeleteType, setSeleteType] = useState([]);
  ...
  const findType = async () => {
    ...
    await fetcherWithToken(url)
      .then((json) => {
        setType(json.data);
        valueTypes = json.data;
      })
      .catch(() => setType([]));
    await fetcherWithToken(urlProduct).then((json) => {
      const findTypes = json.data.product_type_id.map((item) =>
        valueTypes.find((itenFind) => itenFind._id === item)
      );
      setSeleteType(findTypes);
    });
  };
