...
function deleteTypes({ row, fetcherCompany, fetcherWithToken }) {
  const onClickDelete = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        const url = `${process.env.NEXT_PUBLIC_PRODUCT_EXPRESS_BACKEND}/company/${row._id}`;

        await fetcherWithToken(urlImage, {
          method: "DELETE",
          body: JSON.stringify({ images: row.company_image }),
        }).catch(() => {
          Swal.fire({
            icon: "error",
